...
public final class CosineDistance extends AbstractFunction implements StringFunction {
    ...
    public info.debatty.java.stringsimilarity.Cosine getCosineFunction(final Value... values) {
        if(cosine == null) {
            if (values.length == 3) {
    ...
    protected ValueOrError internalEvaluate(final Value... values) {
        ...
        if(assertStringLiteral(values[0]) && assertStringLiteral(values[1])) {
            ...
            return ValueOrError.Double.of(getCosineFunction(values).distance(string1, string2));
        } else {
